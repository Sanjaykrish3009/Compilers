...
AST_Node *new_ast_const_element_node(int const_type, list_t*entry){
	// allocate memory
	AST_Node_Const_Element *v = malloc (sizeof (AST_Node_Const_Element));
...
AST_Node *new_ast_num_node(int const_type, float val){
	// allocate memory
	AST_Node_Num*v = malloc (sizeof (AST_Node_Num));
...
AST_Node *new_ast_element_node(int const_type,AST_Node*atm_no,AST_Node*atm_wt,enum Arithm_op op,AST_Node*charge){
	// allocate memory
	AST_Node_Element *v = malloc (sizeof (AST_Node_Element));
	
	// set entries
	v->type = ELEMENT_NODE;
	v->const_type = const_type;
	v->atm_no=((struct AST_Node_Const*)atm_no)->val;
	
	v->atm_wt=((struct AST_Node_Const*)atm_wt)->val;
	if(op==ADD)
	{
	   v->charge=((struct AST_Node_Num*)charge)->val;
	}
	else
	{ 
	  
	   v->charge= (-1)*((struct AST_Node_Num*)charge)->val;
	}
...
AST_Node *new_ast_molecule_node(int const_type,AST_Node **declarations,int declaration_count,char*name,int num){
	...
	AST_Node_Molecule *v = malloc (sizeof (AST_Node_Molecule));
	
	// set node type
	v->type = MOLECULE_NODE;
	v->const_type = const_type;
	
	AST_Node* declaration=  malloc (sizeof (AST_Node));
		declaration->ele_name=name;
		declaration->num=num;
...
AST_Node *new_ast_attribute_node(list_t *entry,int type){
	// allocate memory
	AST_Node_Attr *v = malloc (sizeof (AST_Node_Attr));
...
AST_Node *new_ast_inbuilt_func_call_node(int func,AST_Node **params, int num_of_pars){
	// allocate memory
	AST_Node_Inbuilt_Func_Call *v = malloc (sizeof (AST_Node_Inbuilt_Func_Call));
...
AST_Node *new_ast_balance_func_call_node(int func,AST_Node **left_params, int left_num_of_pars,AST_Node **right_params, int right_num_of_pars){
	// allocate memory
	AST_Node_Balance_Func_Call *v = malloc (sizeof (AST_Node_Balance_Func_Call));
	
	// set entries
	v->type = BALANCE_FUNC;
	v->func = func;
	v->left_params = left_params;
	v->right_params = right_params;
	v->left_num_of_pars = left_num_of_pars;
	v->right_num_of_pars = right_num_of_pars;
...
AST_Node *new_ast_equ_node(enum Equ_op op, AST_Node *left, AST_Node *right){
	...
	v->data_type = get_result_type(
			expression_data_type(left),  /* data type of left expression  */
			expression_data_type(right), /* data type of right sion */
...
AST_Node *new_ast_mol_node(list_t *entry){
	// allocate memory
	AST_Node_Mol *v = malloc (sizeof (AST_Node_Mol));
...
int expression_data_type(AST_Node *node){
	...
	AST_Node_Const_Element *temp_const_element;
	AST_Node_Func_Call *temp_func_call;
	AST_Node_Inbuilt_Func_Call *temp_inbuilt_func;
	AST_Node_Mol *temp_molecule;
	...
	switch(node->type){
		...
		case ATTR_NODE:
			temp_attr = (AST_Node_Attr *) node;
			if(temp_attr->attr_type==1||temp_attr->attr_type==2)
			{
				return REAL_TYPE;
			}
			else if(temp_attr->attr_type==0)
			{
				return INT_TYPE;
			}
			else
			{
				return temp_attr->entry->st_type;
			}
			break;
		...
		case REF_NODE:    /* identifier reference */
			...
			if(type == INT_TYPE	|| type == REAL_TYPE|| type == STR_TYPE)
			{
		...
		case CONST_NODE:  /* constant */
			...
			break;
		case MOL_NODE:  /* constant */
		       temp_molecule = (AST_Node_Mol *) node;
			return temp_molecule->entry->st_type; 
			break;
			
	        case ELE_NODE:
	         	temp_const_element= (AST_Node_Const_Element *) node;
	         	return temp_const_element->const_type;
	         	break;
		case FUNC_CALL:   /* function call */
			...
			break;
		case INBUILT_FUNC: 
			temp_inbuilt_func=(AST_Node_Inbuilt_Func_Call*)node;
			if(temp_inbuilt_func->func==0)
			{
			//return CHAR_TYPE;
			return STR_TYPE;
			}
			else if(temp_inbuilt_func->func==1)
			{
			return REAL_TYPE;
			}
			else if(temp_inbuilt_func->func==2)
			{
			//return CHAR_TYPE;
			return STR_TYPE;
			}
		...
		default: /* wrong choice case */
			fprintf(stderr, "Error in node selectionn!\n");
			exit(1);
	}
}
...
int getGraphIndex(AST_Node *node){
	...
	switch(node->type){
		...
		case ATTR_NODE:    
			...
			return temp_attr->g_index;
			break;	
		case FUNC_CALL:   /* function call */
			temp_func_call = (AST_Node_Func_Call *) node;
			
			return temp_func_call->g_index; /* return type */
			break;
		default: /* wrong choice case */
...
void ast_print_node(AST_Node *node){
	...
	AST_Node_Incr *temp_incr;
	AST_Node_Func_Call *temp_func_call;
	AST_Node_Inbuilt_Func_Call *temp_inbuilt_func_call;
	AST_Node_Balance_Func_Call *temp_balance_func_call;
	...
	switch(node->type){
		...
		case CONST_NODE:
			...
			break;
		case ELE_NODE:
			temp_const_element = (struct AST_Node_Const_Element *) node;
			if(temp_const_element->const_type==ELEMENT_TYPE)
			{
			printf("Element node %s of Element-type %d is function parameter \n", temp_const_element->entry->st_name,temp_const_element->const_type);
			}
			else
			{
			printf("Molecule node %s of Molecule-type %d is function parameter \n", temp_const_element->entry->st_name,temp_const_element->const_type);
			}
		...
		case FUNC_CALL:
			...
			if(!strcmp(temp_func_call->entry->st_name,"print"))
			{
			 temp_ref=(struct AST_Node_Ref *)temp_func_call->params[0];
			if(temp_func_call->num_of_pars!=1)
			{
			   fprintf(stderr, "ERROR: Function call of %s has wrong number of parameters\n",temp_func_call->entry->st_name);
			exit(1);
			}
			
			}
			
			if(!strcmp(temp_func_call->entry->st_name,"mol_mass"))
			{
			temp_molecule=(struct AST_Node_Mol *)temp_func_call->params[0];
			 if(temp_func_call->num_of_pars!=1)
			 {
			   fprintf(stderr, "ERROR: Function call of %s has wrong number of parameters\n",temp_func_call->entry->st_name);
			exit(1);
			...
			if(temp_molecule->entry->st_type != MOLECULE_TYPE)
			{
			 fprintf(stderr, "ERROR: Function call of %s has wrong datatype of parameters\n",temp_func_call->entry->st_name);
			exit(1);
			}
			...
			break;
		case INBUILT_FUNC:
			temp_inbuilt_func_call = (struct AST_Node_Inbuilt_Func_Call *) node;
			
			if(temp_inbuilt_func_call->func==0)
			{
			    printf("Function Call Node of 'Inbuilt Function - name' with %d parameters\n", temp_inbuilt_func_call->num_of_pars);
			}
			else if(temp_inbuilt_func_call->func==1)
			{
			    printf("Function Call Node of 'Inbuilt Function - molar_mass' with %d parameters\n", temp_inbuilt_func_call->num_of_pars);
			}
			else if(temp_inbuilt_func_call->func==2)
			{
			    printf("Function Call Node of 'Inbuilt Function - emp_for' with %d parameters\n", temp_inbuilt_func_call->num_of_pars);
			}
			
		 	
			break;
		case BALANCE_FUNC:
			temp_balance_func_call = (struct AST_Node_Balance_Func_Call *) node;
			
		 	printf("Function Call Node of 'Inbuilt Function - balance' with %d left parameters and %d right parameters\n", temp_balance_func_call->left_num_of_pars,temp_balance_func_call->right_num_of_pars);
			break;
		case CALL_PARAMS:
		...
		case ATTR_NODE:
		 	...
		 	if(temp_attr->attr_type==0)
		 	{
		 	    printf("Element Attribute Node of entry Atomic number\n");
		 	}
		 	else if(temp_attr->attr_type==1)
		 	{
		 	    printf("Element Attribute Node of entry Atomic weight\n");
		 	}
		 	else if(temp_attr->attr_type==2)
		 	{
		 	    printf("Element Attribute Node of entry Charge\n");
		 	}
			break;
		
		case MOL_NODE:
			temp_molecule = (struct AST_Node_Mol *) node;
			printf("Molecule Node of entry %s\n", temp_molecule->entry->st_name);
			break;
...
void ast_traversal(AST_Node *node){
	...
	else if(node->type==INBUILT_FUNC)
	{
		AST_Node_Inbuilt_Func_Call *temp_func_call = (struct AST_Node_Inbuilt_Func_Call *) node;
	...
	else if(node->type==BALANCE_FUNC)
	{
		AST_Node_Balance_Func_Call *temp_func_call = (struct AST_Node_Balance_Func_Call *) node;
		
		ast_print_node(node);
		
		if(temp_func_call->left_num_of_pars != 0&&temp_func_call->right_num_of_pars != 0){
			printf("Call parameters on left side of Equation:\n");
			
			for(i = 0; i < temp_func_call->left_num_of_pars; i++){
				ast_traversal(temp_func_call->left_params[i]);
			}
			
			printf("Call parameters on right side of Equation:\n");
			
			for(i = 0; i < temp_func_call->right_num_of_pars; i++){
				ast_traversal(temp_func_call->right_params[i]);
			}
		}
	}
