  ...
  for (i=0; i < SIZE; ++i){ 
	if (hash_table[i] != NULL){ 
		...
		while (l != NULL){ 
			...
			else if (l->st_type == STR_TYPE)          fprintf(of,"%-15s","string");
			else if (l->st_type == VOID_TYPE)          fprintf(of,"%-15s","void");
			else if (l->st_type == ELEMENT_TYPE)          fprintf(of,"%-15s","element");
			else if (l->st_type == MOLECULE_TYPE)          fprintf(of,"%-15s","molecule");
			else if (l->st_type == FUNCTION_TYPE){
				fprintf(of,"func ret ");
				if (l->inf_type == INT_TYPE) 		   fprintf(of,"%-6s","int");
				else if (l->inf_type  == REAL_TYPE)    fprintf(of,"%-6s","real");
				else if (l->inf_type  == STR_TYPE)	   fprintf(of,"%-6s","string");
				else if (l->inf_type  == VOID_TYPE)	   fprintf(of,"%-6s","void");
				else if (l->inf_type  == ELEMENT_TYPE)    fprintf(of,"%-6s","element");
				else if (l->inf_type  == MOLECULE_TYPE)    fprintf(of,"%-6s","molecule");
...
int get_type(char *name){ // get the type of an entry
	...
	/* if "simple" type */
	if(l->st_type == INT_TYPE || l->st_type == REAL_TYPE || l->st_type == STR_TYPE|| l->st_type == ELEMENT_TYPE|| l->st_type ==MOLECULE_TYPE){
		return l->st_type;
	}
	/* function */
	else{
